...
- 모든 path input은 workspace-relative path로 해석합니다.
- absolute path, workspace 밖 traversal, workspace 밖 symlink escape를 거부합니다.
...
| `workbench.run_extract` | yes | `risu-core extract`를 안전 게이트 뒤에서 실행합니다. `sourcePath`는 기존 workspace-relative 파일, `outDir`은 신규 workspace-relative 디렉터리여야 합니다. |
