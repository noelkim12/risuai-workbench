...
describe('handleRouteIntent authoring skills', () => {
  it('recommends authoring skill selection for new RisuAI system design requests', async () => {
    ...
    expect(route.recommendedTools).toContain('workbench.recommend_skills');
  ...
  it('does not recommend skill application for pure validation requests', async () => {
    const result = await handleRouteIntent({
