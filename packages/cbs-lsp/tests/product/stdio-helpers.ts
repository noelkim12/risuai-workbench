/**
 * Shared stdio LSP client helpers for product-level E2E tests.
 * @file packages/cbs-lsp/tests/product/stdio-helpers.ts
 */
...
import { spawnCliProcess } from './test-helpers';
...
export interface JsonRpcResponseRecord {
  error?: JsonRpcResponseMessage['error'];
  id: number | null;
  result?: unknown;
}

export interface JsonRpcNotificationRecord {
  method: string;
  params: unknown;
}
...
export class StdioLspClient {
  ...
  /**
   * requestChunked 함수.
   * request frame을 여러 stdin chunk로 쪼개내면서 응답을 기다림.
   *
   * @param method - 호출할 LSP method
   * @param params - method에 전달할 params payload
 ...
 */
export function createStdioClient(args: readonly string[]): StdioLspClient {
 ...
 */
export function positionAt(text: string, needle: string, characterOffset: number = 0): { character: number; line: number } {
 ...
 */
export function getHoverMarkdown(hover: unknown): string | null {
 ...
 */
export async function requestHoverUntilReady(client: StdioLspClient, uri: string, text: string): Promise<unknown> {
 ...
 */
export async function requestReferencesUntil(
 ...
 * textDocument/publishDiagnostics notification이 predicate를 만족할 때까지 기다림.
 *
 ...
 * @returns predicate를 만족한 diagnostics params
 */
export async function waitForDiagnosticsUntil(
  client: StdioLspClient,
  uri: string,
  ...
  const waitDetail = lastWaitError ? ` Last wait error: ${lastWaitError.message}.` : '';
  throw new Error(`Timed out waiting for diagnostics for ${uri}.${waitDetail} stderr: ${client.getStderr()}`);
}
