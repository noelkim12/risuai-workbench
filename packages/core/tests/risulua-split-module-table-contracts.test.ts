...
describe('risulua-split module-table contracts', () => {
  it('defines approved MVP paths, classifier precedence, host effect classes, and reason codes', () => {
    ...
    expect(RISULUA_MODULE_TABLE_BUTTON_ACTIONS_PATH).toBe('lua/button_actions/actions.risulua');
    expect(RISULUA_MODULE_TABLE_VARIABLE_STORE_PATH).toBe('lua/state/variable_store.risulua');
    expect(RISULUA_MODULE_TABLE_PROMPT_STORE_PATH).toBe('lua/prompts/instruction_store.risulua');
    expect(RISULUA_MODULE_TABLE_RUNTIME_OUTPUT_PATH).toBe('lua/runtime/output.risulua');
    ...
    expect(RISULUA_MODULE_TABLE_MVP_ARTIFACT_PATHS).toContain(RISULUA_MODULE_TABLE_PROMPT_STORE_PATH);
    expect(RISULUA_MODULE_TABLE_MVP_ARTIFACT_PATHS).toContain(RISULUA_MODULE_TABLE_BUTTON_ACTIONS_PATH);
  ...
  it('allows generated domain targets only when candidate generation metadata matches', () => {
    const refactorMap = createValidRefactorMap();
    const domainPath = 'lua/domain/deck.risulua';
    const invalidMap: RisuLuaModuleTableRefactorMapContract = {
      ...refactorMap,
      modules: [
        ...refactorMap.modules,
      ...
      symbols: refactorMap.symbols.map((symbol) => symbol.id === 'symbol:common:trim'
        ? { ...symbol, classification: 'extract:domain-function', targetModule: domainPath }
        : symbol),
    ...
    expect(refactorMap.domainCandidates[0]).toEqual(expect.objectContaining({
      recommendedPath: domainPath,
      autoGenerated: false,
    }));
    expect(isForbiddenRisuLuaModuleTableMvpTarget(domainPath)).toBe(false);
    expect(isAllowedRisuLuaModuleTableMvpTarget(domainPath)).toBe(true);
    expect(findings).toEqual(expect.arrayContaining([
      expect.objectContaining({
        code: 'invalid-domain-candidate',
        candidateName: 'deck',
        message: 'Non-generated domain candidate deck must have autoGenerated set to false.',
      }),
    ]));

    const validGeneratedMap: RisuLuaModuleTableRefactorMapContract = {
      ...invalidMap,
      domainCandidates: refactorMap.domainCandidates.map((candidate) => ({
        ...candidate,
        generationStatus: 'generated',
        generatedPath: domainPath,
        generationBlockedReasons: [],
        notGeneratedReason: '',
        autoGenerated: true,
      })),
    };
    expect(validateRisuLuaModuleTableRefactorMap(validGeneratedMap)).toEqual([]);
  });

  it('accepts optional domain candidate grouping metadata', () => {
    const refactorMap: RisuLuaModuleTableRefactorMapContract = {
      ...createValidRefactorMap(),
      domainCandidates: [{
        name: 'scoreDeck',
        sourceSymbols: ['scoreDeck'],
        sourceRanges: [{ startLine: 1, endLine: 3, startOffset: 0, endOffset: 60 }],
        confidence: 0.7,
        evidence: ['Grouped into lua/domain/deck.risulua because strong token "deck" appears in 2 generated domain candidates.'],
        recommendedPath: 'lua/domain/deck.risulua',
        generationStatus: 'generated',
        generatedPath: 'lua/domain/deck.risulua',
        generationBlockedReasons: [],
    ...
    domainCandidates: [
      {
        ...
        generationBlockedReasons: ['Domain grouping is report-only in the module-table MVP.'],
        hostEffects: createEmptyRisuLuaModuleTableHostEffects(),
