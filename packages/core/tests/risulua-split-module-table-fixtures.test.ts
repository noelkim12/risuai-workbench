...
describe('risulua-split module-table fixture matrix', () => {
  ...
  it('asserts forbidden feature helpers and generated domain modules never appear as fixture targets', () => {
    const refactorMap = createMatrixRefactorMap();
    const targetModules = [
    ...
    expect(targetModules.every((target) => !/^lua\/domain\/[^/]+\.risulua$/.test(target))).toBe(true);
    expect(isForbiddenRisuLuaModuleTableMvpTarget('lua/features/output_helpers.risulua')).toBe(true);
    expect(isForbiddenRisuLuaModuleTableMvpTarget('lua/domain/score_deck.risulua')).toBe(true);
  });
