...
const MODULE_TABLE_FIXTURE_A_ROOT = resolvePrivateFixturePath(
  'RISU_WORKBENCH_MODULE_TABLE_FIXTURE_A',
  'module-table-sample-a',
);
...
const FIXTURE_MATRIX: ModuleTableFixtureCase[] = [
  {
    ...
    expectedSymbols: [
      symbol(
        'symbol:common:trimText',
        'trimText',
        'extract:pure-helper',
        RISULUA_MODULE_TABLE_COMMON_HELPERS_PATH,
      ),
      symbol(
        'symbol:host-global:formatPublicName',
        'formatPublicName',
      ...
      symbol(
        'symbol:host-async:sendAsyncNotice',
        'sendAsyncNotice',
        'bridge:host-visible-global',
        RISULUA_MODULE_TABLE_ASYNC_ACTIONS_PATH,
        {
  ...
  },
  {
    ...
    expectedSymbols: [
      symbol(
        'symbol:start:prepareTitle',
        'prepareTitle',
        'extract:pure-helper',
        START_HANDLER_HELPERS_PATH,
      ),
      symbol(
        'symbol:input:readMood',
        'readMood',
      ...
      symbol(
        'symbol:button:buttonLabel',
        'buttonLabel',
        'extract:pure-helper',
        BUTTON_HANDLER_HELPERS_PATH,
      ),
      symbol(
        'symbol:listen-edit:normalizeProfileName',
  ...
  },
  {
    ...
    expectedPreserved: [
      ...
      preserved(
        'preserve:output:controlledCapture',
        'controlledCapture',
  ...
  },
  {
    ...
    expectedSymbols: [
      symbol(
        'symbol:duplicate:duplicatedGlobal__L1',
        'duplicatedGlobal',
        'bridge:host-visible-global',
        RISULUA_MODULE_TABLE_DUPLICATE_GLOBALS_PATH,
        {
      ...
      symbol(
        'symbol:duplicate:duplicatedGlobal__L2',
        'duplicatedGlobal',
        'bridge:host-visible-global',
        RISULUA_MODULE_TABLE_DUPLICATE_GLOBALS_PATH,
        {
    ...
    expectedPreserved: [
      preserved(
        'preserve:dynamic:buttonMarkup',
        'risu-btn',
        'preserve:dynamic-global-reference-risk',
      ),
      preserved(
        'preserve:dynamic:buttonString',
        'dynamicButton',
        'preserve:dynamic-global-reference-risk',
      ),
  ...
  },
  {
    ...
    expectedSymbols: [
      symbol(
        'symbol:domain-candidate:scoreDeck',
        'scoreDeck',
...
describe('risulua-split module-table fixture matrix', () => {
  it('defines deterministic contract fixtures using only approved module-table targets', () => {
    ...
    expect(APPROVED_MODULE_TABLE_TARGETS).toEqual(
  ...
  it('validates a combined refactor-map fixture for future analyzer/classifier/writer tasks', () => {
    ...
    expect(refactorMap.modules.map((moduleContract) => moduleContract.path)).toEqual(
      APPROVED_MODULE_TABLE_TARGETS.slice(0, -1),
    );
    expect(refactorMap.symbols).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          originalName: 'formatPublicName',
          targetModule: RISULUA_MODULE_TABLE_GLOBAL_FUNCTIONS_PATH,
        }),
        expect.objectContaining({
          originalName: 'sendAsyncNotice',
          targetModule: RISULUA_MODULE_TABLE_ASYNC_ACTIONS_PATH,
        }),
        expect.objectContaining({
          originalName: 'prepareTitle',
          targetModule: START_HANDLER_HELPERS_PATH,
        }),
        expect.objectContaining({
          originalName: 'readMood',
          targetModule: INPUT_HANDLER_HELPERS_PATH,
        }),
        expect.objectContaining({
          originalName: 'readByName',
          targetModule: OUTPUT_HANDLER_HELPERS_PATH,
        }),
        expect.objectContaining({
          originalName: 'buttonLabel',
          targetModule: BUTTON_HANDLER_HELPERS_PATH,
        }),
        expect.objectContaining({
          originalName: 'normalizeProfileName',
          targetModule: LISTEN_EDIT_HANDLER_HELPERS_PATH,
        }),
        expect.objectContaining({
          originalName: 'scoreDeck',
          targetModule: RISULUA_MODULE_TABLE_DOMAIN_CANDIDATES_PATH,
        }),
        expect.objectContaining({
    ...
    expect(refactorMap.preserved.map((entry) => entry.originalName)).toEqual(
      expect.arrayContaining([
        'onOutput',
        'onInput',
        'onStart',
  ...
  it('asserts forbidden feature helpers never appear and domain modules remain opt-in fixture targets', () => {
    ...
    const targetModules = [
      ...
      ...refactorMap.symbols
        .map((symbolContract) => symbolContract.targetModule)
        .filter((path): path is string => path !== undefined),
      ...refactorMap.domainCandidates.map((candidate) => candidate.recommendedPath),
    ];

    expect(
      targetModules.every((target) => !/^lua\/features\/[^/]+_helpers\.risulua$/.test(target)),
    ).toBe(true);
    expect(isForbiddenRisuLuaModuleTableMvpTarget('lua/features/output_helpers.risulua')).toBe(
      true,
    );
  ...
  it('keeps comment and string false positives out of executable parser ranges', async () => {
    ...
    expect(result.nonExecutableRanges.map((range) => range.text).join('\n')).toEqual(
      expect.stringContaining('listenEdit'),
    );
    expect(result.nonExecutableRanges.map((range) => range.text).join('\n')).toEqual(
      expect.stringContaining('onOutput'),
    );
    expect(result.nonExecutableRanges.map((range) => range.text).join('\n')).toEqual(
      expect.stringContaining('commentedDeclaration'),
    );
  ...
  it('fails closed for malformed snippets and treats comment-only snippets as no-eligible fixtures', async () => {
    ...
    const commentOnly = await parseRisuLuaModuleTableSource(
      lines([
  ...
  it('keeps private module-table fixture main as an ABI shell with listener shims after fixture generation', () => {
    const mainPath = path.join(MODULE_TABLE_FIXTURE_A_ROOT, 'lua', 'main.risulua');
    if (!existsSync(mainPath)) {
      console.log('Skipping: private module-table fixture A main file not found.');
      return;
    }
  ...
  it('keeps export manifests in regenerated module-table fixtures', () => {
    const fixtureManifestPaths = [MODULE_TABLE_FIXTURE_A_ROOT, MODULE_TABLE_FIXTURE_B_ROOT].map(
      (root) => path.join(root, 'docs', 'risulua-export-manifest.json'),
    );
    if (fixtureManifestPaths.some((manifestPath) => !existsSync(manifestPath))) {
      console.log('Skipping: private module-table fixture manifests not found.');
      return;
    }
  ...
  it('asserts Gacha Island module-table fixture has store-backed public data alias and generated domain clusters', () => {
    const mainPath = path.join(GACHA_ISLAND_FIXTURE_ROOT, 'lua', 'main.risulua');
    if (!existsSync(mainPath)) {
      console.log('Skipping: Gacha Island fixture main file not found.');
      return;
    }
  ...
  it('asserts Gacha Island domain-candidates have expected generation status for key clusters', () => {
    const candidatesPath = path.join(GACHA_ISLAND_FIXTURE_ROOT, 'docs', 'domain-candidates.json');
    if (!existsSync(candidatesPath)) {
      console.log('Skipping: Gacha Island fixture domain-candidates not found.');
      return;
    }
  ...
  it('asserts Gacha Island generated domain modules exist for key clusters', () => {
    ...
    if (!existsSync(domainDir)) {
      console.log('Skipping: Gacha Island fixture domain directory not found.');
      return;
    }
...
function expectParseSuccess(
  result: RisuLuaModuleTableParseResult,
): asserts result is RisuLuaModuleTableParseSuccess {
...
function createMatrixRefactorMap(): RisuLuaModuleTableRefactorMapContract {
  const symbols = FIXTURE_MATRIX.flatMap((fixture, fixtureIndex) =>
    fixture.expectedSymbols.map((expectedSymbol, symbolIndex) => ({
      id: expectedSymbol.id,
      originalName: expectedSymbol.originalName,
      declarationKind:
        expectedSymbol.classification === 'report:domain-candidate'
          ? ('domain-candidate' as const)
          : ('nested-local-function' as const),
      ...
      bridge: expectedSymbol.globalBridge
        ? {
            ...
            moduleAlias:
              expectedSymbol.targetModule === RISULUA_MODULE_TABLE_ASYNC_ACTIONS_PATH
                ? '__async_actions'
                : expectedSymbol.targetModule === RISULUA_MODULE_TABLE_DUPLICATE_GLOBALS_PATH
                  ? '__duplicate_globals'
                  : '__global_functions',
            exportName: expectedSymbol.originalName,
            mainAssignment: {
  ...
  return {
    ...
    symbols,
    preserved: FIXTURE_MATRIX.flatMap((fixture, fixtureIndex) =>
      fixture.expectedPreserved.map((expectedPreserved, preservedIndex) => ({
