...
| **Lua Function** | Lua 스크립트 내 함수 단위 로직 | `setState`/`getState`로 변수를 읽고 쓰며, `getLoreBooks` API로 lorebook에 직접 접근 |
...
Lua 함수가 `getLoreBooks` API를 호출하여 특정 lorebook 엔트리에 직접 접근하는 관계입니다.

- **분석 소스**: `LuaAnalysisArtifact.lorebookCorrelation.loreApiCalls`
- **레이블**: `getLoreBooks`에 전달된 keyword 문자열
...
Lua Function ──lore-direct──> Lorebook     (getLoreBooks API 호출)
