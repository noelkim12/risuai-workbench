...
export interface CharxWikiOptions {
  extractDir: string;
  wikiRoot?: string;
  artifactKey?: ArtifactKey;
}

/** Run the wiki renderer for a single charx artifact. */
export function runCharxWiki(data: CharxReportData, options: CharxWikiOptions): void {
  ...
  const artifactKey =
    options.artifactKey ?? defaultArtifactKey(path.basename(resolvedExtractDir), 'character');
  ...
  const files = renderArtifactWiki(data, ctx);
  ...
  const section = buildArtifactsSection(listings, workspace);
  rewriteIndexArtifactsSection(path.join(wikiRoot, '_index.md'), section);
