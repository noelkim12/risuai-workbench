...
<body>
  <div class="max-w-[1480px] mx-auto px-4 py-5 pb-9 lg:px-10 lg:py-9 lg:pb-14 animate-fade-in font-sans">
    ...
    <div class="flex flex-wrap gap-2.5 mb-5" aria-label="Severity filters">
      <button type="button" class="sev-chip active" data-severity="all">${escapeHtml(t(locale, 'shell.filter.all'))}</button>
      <button type="button" class="sev-chip" data-severity="error">${escapeHtml(t(locale, 'shell.filter.error'))}</button>
      <button type="button" class="sev-chip" data-severity="warning">${escapeHtml(t(locale, 'shell.filter.warning'))}</button>
      <button type="button" class="sev-chip" data-severity="info">${escapeHtml(t(locale, 'shell.filter.info'))}</button>
    </div>
...
function renderFindingsPanel(doc: AnalysisVisualizationDoc, panel: FindingsPanel, locale: Locale): string {
  ...
  return `<article class="glass glass-gradient relative p-[22px] mb-[18px] last:mb-0" data-panel-kind="findings" data-panel-id="${escapeHtml(panel.id)}"><div class="flex flex-wrap items-baseline justify-between gap-2.5 mb-4 relative"><div><h3 class="mb-0 tracking-tight before:content-['◆'] before:mr-2 before:text-[0.7em] before:opacity-40 before:align-middle">${escapeHtml(panel.title)}</h3>${panel.description ? `<p class="m-0 text-text-muted">${escapeHtml(panel.description)}</p>` : ''}</div></div><div class="grid gap-3">${panel.findings
    .map((finding) => {
