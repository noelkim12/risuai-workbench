...
function inferLuaFunctionName(code: string): string | null {
  if (!code) return null;
  const patterns = [
...
function getLorebookFolderKey(entry: any): string | null {
  if (!entry || !Array.isArray(entry.keys) || entry.keys.length === 0) return null;
  const key = entry.keys[0];
  return typeof key === 'string' && key.length > 0 ? key : null;
}

function createLorebookDirAllocator(): (parentRelDir: string, rawName: string) => string {
...
function buildLorebookFolderDirMap(
  entries: any[],
...
function extractLorebookRows(
  ...
  const fallbackFolderMap = buildFolderMap(entries as any);
