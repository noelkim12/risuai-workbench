...
### 5. pack 워크플로우는 모든 아티팩트 유형을 지원한다

`pack` 커맨드는 character, module, preset 모두를 지원한다:

- **character**: `character/` (with `metadata.json` inside) + 선택적 `lorebooks/`, `regex/`, `lua/`, `assets/`, `html/`, `variables/` → `.charx`, `.png`, `charx-jpg` 출력
- **module**: `metadata.json` + `lorebooks/` + 선택적 `regex/`, `lua/`, `assets/`, `html/`, `variables/`, `toggle/` → `.json` (기본), `.risum` 출력
- **preset**: `metadata.json` + `prompts/` + 선택적 `prompt_template/`, `parameters.json`, `model.json`, `provider/`, `regex/`, `advanced.json` → `.json`, `.risup` 출력
...
1. `metadata.json` + `lorebooks/` → module (strictest criteria, checked first)
2. `metadata.json` + (`prompts/` | `prompt_template/` | `regex/` | `model.json` | `parameters.json` | `instruct_settings.json`) → preset
3. `character/` + `lorebooks/` → charx

Module takes precedence over preset when both markers are present (e.g., `metadata.json + lorebooks/ + regex/` → module).

Analyze workflow uses canonical-marker-first auto-detection. While canonical workspace markers (character/, lorebooks/, metadata.json, prompts/, etc.) are preferred, legacy root-JSON fallback (`charx.json`, `module.json`, `preset.json`) remains supported in analyze/compose workflows. Strict eradication is deferred.
...
│
├── pack/
│   ├── workflow.ts           유형 라우팅 (character/module/preset)
│   ├── utils.ts              패킹 유틸리티
│   ├── character/
│   │   └── workflow.ts       캐릭터 카드 재조립 오케스트레이션
│   ├── module/
│   │   └── workflow.ts       모듈 재조립 오케스트레이션
│   └── preset/
│       └── workflow.ts       프리셋 재조립 오케스트레이션
│
├── analyze/
