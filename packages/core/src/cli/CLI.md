...
`analyze` 커맨드는 `--type lua`, `--type charx`, `--type module`, `--type preset`, `--type compose`로 분석 유형을 명시할 수 있다. 생략 시 대상을 자동 감지한다 (`.lua` 파일 → lua, `module.json` 디렉토리 → module, `preset.json` 디렉토리 → preset, `charx.json` 디렉토리 → charx). `compose`는 자동 감지하지 않고 명시적으로만 진입한다.
