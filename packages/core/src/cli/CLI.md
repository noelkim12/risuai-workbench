...
- **module**: `.risumodule` + 선택적 `lorebooks/`, `regex/`, `lua/`, `assets/`, `html/`, `variables/`, `toggle/` → `.json` (기본), `.risum` 출력
...
Module takes precedence over preset when both markers are present (e.g., `.risumodule + metadata.json + regex/` → module).

Analyze workflow uses canonical-marker-first auto-detection. While canonical workspace markers (`.risumodule`, `character/`, `lorebooks/`, `metadata.json`, `prompts/`, etc.) are preferred, legacy root-JSON fallback (`charx.json`, `module.json`, `preset.json`) remains supported in analyze/compose workflows where explicitly documented. Module `metadata.json` fallback is removed.
