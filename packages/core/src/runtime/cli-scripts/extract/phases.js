...
/**
 * Infer a human-readable function name from Lua source code.
 * Checks patterns: `function NAME(`, `local function NAME(`, `NAME = function(`.
 * Returns the first match or null.
 */
function inferLuaFunctionName(code) {
  if (!code) return null;
  const patterns = [
  ...
  for (let i = 0; i < triggers.length; i++) {
    ...
    for (let j = 0; j < effects.length; j++) {
      ...
      if (effect.type === "triggerlua" && effect.code) {
        triggerCount++;
        const baseName = sanitizeFilename(trigger.comment || inferLuaFunctionName(effect.code) || `trigger_${i}`);
