...
function renderHtml(data, outputDir) {
  ...
  const sharedRows = sharedVars.length ? sharedVars.map(v => {
    const lorebookEntries = v.lorebookEntries || v.lorebook || [];
    const regexScripts = v.regexScripts || v.regex || [];
    return `
      <tr>
        <td><code>${h(v.varName)}</code></td>
        <td class="small-text">${lorebookEntries.map(h).join(', ')}</td>
        <td class="small-text">${regexScripts.map(h).join(', ')}</td>
      </tr>
    `;
