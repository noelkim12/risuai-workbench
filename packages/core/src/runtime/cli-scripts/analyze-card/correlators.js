...
function buildUnifiedCBSGraph(allCollected, defaultVariables) {
  ...
  for (const element of allCollected || []) {
    ...
    for (const varName of reads || []) {
      ...
      // Use per-variable owner labels when available (e.g. Lua analysis), else file-level elementName
      const labels = readersByVar && readersByVar[varName] && readersByVar[varName].length > 0
    ...
    for (const varName of writes || []) {
      ...
      ensureSource(entry, elementType);
      // Use per-variable owner labels when available (e.g. Lua analysis), else file-level elementName
      const labels = writersByVar && writersByVar[varName] && writersByVar[varName].length > 0
