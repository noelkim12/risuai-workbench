...
async function main() {
  ...
  try { lorebookCBS = collectLorebookCBS(card, resolvedOutDir); } catch (e) { console.warn(`  ⚠️ Lorebook CBS 수집 실패: ${e.message}`); lorebookCBS = []; }
  try { regexCBS = collectRegexCBS(card, resolvedOutDir); } catch (e) { console.warn(`  ⚠️ Regex CBS 수집 실패: ${e.message}`); regexCBS = []; }
  try { variablesResult = collectVariablesCBS(card, resolvedOutDir); } catch (e) { console.warn(`  ⚠️ Variables 수집 실패: ${e.message}`); variablesResult = { variables: {}, cbsData: [] }; }
  try { htmlResult = collectHTMLCBS(card, resolvedOutDir); } catch (e) { console.warn(`  ⚠️ HTML CBS 수집 실패: ${e.message}`); htmlResult = { cbsData: null, assetRefs: [] }; }
