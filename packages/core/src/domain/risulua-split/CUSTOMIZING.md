...
`preserve:captures-mutable-state`가 남아 있는 report를 읽을 때는 남은 클러스터가 실제 미분할 구현인지, 아니면 의도적으로 `lua/main.risulua`에 유지한 host ABI bridge인지 먼저 구분합니다. Private module-table sample 같은 대형 스크립트에서는 main에 남은 public 이름이 모두 실패가 아니라, host가 호출해야 하는 ABI shell일 수 있습니다.
...
Common Private module-table sample-style blockers:
