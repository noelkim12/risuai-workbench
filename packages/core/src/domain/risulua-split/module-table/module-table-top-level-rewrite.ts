...
export interface ModuleBodyPlan {
  ...
  requireId: string;
  alias: string;
  category: 'common-helper' | 'host-global';
  body: string;
  exportNames: string[];
...
export function planTopLevelRewrite(input: TopLevelRewriteInput): TopLevelRewriteResult {
  ...
  const task10Bindings = editPlan.mainRequireBindings.filter(
    (b) => TASK_10_MODULE_PATHS.has(b.targetModule),
  );
...
function buildMainRewritePlan(
  ...
  const shadowedScopes = detectShadowedScopes(source, helperNames, extractionRanges, nonExecIndex);
  ...
  for (const sym of refactorMap.symbols) {
    if (isExtracted(sym)) {
      extractions.push({
        ...
        isHostGlobal: sym.classification === 'bridge:host-visible-global',
...
function isExtracted(sym: RisuLuaModuleTableSymbolContract): boolean {
  if (!sym.targetModule || !TASK_10_MODULE_PATHS.has(sym.targetModule)) return false;
