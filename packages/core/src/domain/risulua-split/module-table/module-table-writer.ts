...
export interface RisuLuaModuleTablePublicDataDeclaration {
  name: string;
  text: string;
  initializerText: string;
  startOffset: number;
...
async function runPipeline(input: CreateRisuLuaModuleTableArtifactsInput): Promise<PipelineResult> {
  ...
  const localDeclarations = findTopLevelLocalTableDeclarations(input.source);
  const publicDataDeclarations = findTopLevelPublicDataDeclarations(input.source);
  const variableStoreNames = [...localDeclarations, ...publicDataDeclarations].sort((left, right) => left.startOffset - right.startOffset).map((declaration) => declaration.name);
...
function renderVariableStoreEntry(
  declaration: VariableStoreDeclaration,
...
type VariableStoreDeclaration = RisuLuaModuleTableLocalTableDeclaration | RisuLuaModuleTablePublicDataDeclaration;

function extractVariableStore(mainText: string): VariableStoreExtractionResult {
  const localDeclarations = findTopLevelLocalTableDeclarations(mainText);
  const publicDataDeclarations = findTopLevelPublicDataDeclarations(mainText);
  const declarations: VariableStoreDeclaration[] = [...localDeclarations, ...publicDataDeclarations].sort((left, right) => left.startOffset - right.startOffset);
  if (declarations.length === 0) return { mainText, exportNames: [] };

  const publicDataNames = new Set(publicDataDeclarations.map((declaration) => declaration.name));
  ...
  for (const declaration of declarations) {
    ...
    const isPublicData = publicDataNames.has(declaration.name);
    const replacementText = isPublicData
      ? `${declaration.name} = ${VARIABLE_STORE_ALIAS}.${declaration.name}\n`
      : `local ${declaration.name} = ${VARIABLE_STORE_ALIAS}.${declaration.name}\n`;
    replacements.push({
...
export function findTopLevelPublicDataDeclarations(text: string): RisuLuaModuleTablePublicDataDeclaration[] {
  const declarations: RisuLuaModuleTablePublicDataDeclaration[] = [];
  let body: LuaNode[];
  try {
    body = parseLuaBody(text);
  } catch {
...
function isSinglePublicDataStoreStatement(node: LuaNode): node is LuaAssignmentStatement {
  if (node.type !== 'AssignmentStatement') return false;
  const statement = node as LuaAssignmentStatement;
