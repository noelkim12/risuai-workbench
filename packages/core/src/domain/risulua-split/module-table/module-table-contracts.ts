...
export type RisuLuaDomainGroupingReason =
  | 'singleton'
  | 'repeated-token'
  | 'normalized-token'
  | 'cluster-policy'
...
export interface RisuLuaDomainGroupingMetadata {
  reason: RisuLuaDomainGroupingReason;
  path: string;
  token?: string;
  family?: 'array' | 'text' | 'number' | 'render';
  peers: string[];
}
...
export function validateRisuLuaModuleTableDomainCandidates(
  ...
  for (const candidate of candidates) {
    ...
    if (candidate.grouping !== undefined) {
      const allowedReasons: RisuLuaDomainGroupingReason[] = [
        'singleton',
        'repeated-token',
        'normalized-token',
        'cluster-policy',
      ...
      if (!allowedReasons.includes(candidate.grouping.reason)) {
        findings.push({
          code: 'invalid-domain-candidate',
          message: `Domain candidate ${candidate.name} has invalid grouping reason ${candidate.grouping.reason}.`,
          candidateName: candidate.name,
        });
      }
      if (candidate.grouping.path !== candidate.recommendedPath) {
        findings.push({
          code: 'invalid-domain-candidate',
          message: `Domain candidate ${candidate.name} grouping path must match recommendedPath.`,
          candidateName: candidate.name,
        });
      }
      if (candidate.grouping.peers.length === 0) {
        findings.push({
          code: 'invalid-domain-candidate',
          message: `Domain candidate ${candidate.name} grouping metadata must include peers.`,
          candidateName: candidate.name,
        });
      }
    }
