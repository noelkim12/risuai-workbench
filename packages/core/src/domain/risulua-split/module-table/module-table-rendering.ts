...
export interface RisuLuaModuleTableRefactorMapDocument {
  version: 1;
  mode: 'module-table';
  domainGeneration?: RisuLuaModuleTableRefactorMapContract['domainGeneration'];
  sourceFile: string;
  source: {
    file: string;
  };
...
export function createRisuLuaModuleTableRefactorMapDocument(
  ...
  return {
    version: 1,
    mode: 'module-table',
    domainGeneration: refactorMap.domainGeneration,
...
export function renderRisuLuaModuleTableReportSections(
  ...
  return [
    ...
    '',
    ...renderAbiBridgeClusterGuidance(document),
...
function renderDomainCandidateSummaries(candidates: RisuLuaModuleTableDomainCandidateContract[]): string[] {
  ...
  return candidates.map((candidate) => `- \`${candidate.name}\` confidence ${candidate.confidence}; proposed target \`${candidate.recommendedPath}\`; generationStatus=${candidate.generationStatus}; autoGenerated=${String(candidate.autoGenerated)}; evidence: ${candidate.evidence.join(', ')}`);
}

function renderAbiBridgeClusterGuidance(document: RisuLuaModuleTableRefactorMapDocument): string[] {
  const hasMutableCapturePreserves = document.preserved.some((entry) => entry.reason === 'preserve:captures-mutable-state');
  if (!hasMutableCapturePreserves) return [];

  const candidates = document.domainCandidates.filter((candidate) => isClosureClusterGuidanceCandidate(candidate));
  ...
  return [
    ...
    ...candidates.map((candidate) => `- \`${candidate.name}\`: generationStatus=\`${candidate.generationStatus}\`; generationBlockedReasons=${inlineCodeList(candidate.generationBlockedReasons)}; recommendedPath=\`${candidate.recommendedPath}\`.`),
    '',
  ];
}

function isClosureClusterGuidanceCandidate(candidate: RisuLuaModuleTableDomainCandidateContract): boolean {
  const evidence = candidate.evidence.join(' ');
  return candidate.recommendedPath.includes('/domain/')
    || candidate.recommendedPath.includes('/state/variable_store.risulua')
    || evidence.includes('cluster')
    || evidence.includes('store-backed')
    || candidate.generationBlockedReasons.length > 0;
}
