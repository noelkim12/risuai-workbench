...
- `src/node/index.ts`
- filesystem 헬퍼, PNG chunk 헬퍼, `parseCardFile`를 다시 export
...
- `src/cli/main.ts`가 `extract`, `pack`, `analyze`, `analyze-card`, `build` 서브커맨드를 디스패치
...
- `src/types/card.ts`는 `CardData`, `RegexScript`, `LorebookEntry`, `Variable` 같은 핵심 데이터 형태를 정의
...
- `card-io.ts`: 디스크의 `.json`, `.png` 카드 입력 파싱
...
- command별 workflow 디렉토리
  ...
  - `analyze-card/`
    - `workflow.ts`: 카드 전체 분석용 end-to-end 오케스트레이터
