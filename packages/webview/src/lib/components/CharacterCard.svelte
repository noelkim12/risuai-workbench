<script lang="ts">
  import type { BrowserArtifactCard, CharacterBrowserCard, ModuleBrowserCard } from '../types';
  // biome-ignore lint/correctness/noUnusedImports: Svelte markup consumes this component.
  import StatusBadge from './StatusBadge.svelte';
  // biome-ignore lint/correctness/noUnusedImports: Svelte markup consumes this component.
  import TagChip from './TagChip.svelte';

  export let card: BrowserArtifactCard;
  ...
  $: imageUri = card.artifactKind === 'character' ? card.imageUri : undefined;
  $: if (imageUri) {
  ...
  /**
   * getModuleIcon 함수.
   * Module source format을 카드 제목 앞의 작은 시각 식별자로 변환함.
   *
   * @param format - module source format 값
   * @returns 카드 제목 행에 표시할 unicode icon
   */
  // biome-ignore lint/correctness/noUnusedVariables: Svelte markup calls this source icon helper.
  function getModuleIcon(format: ModuleBrowserCard['sourceFormat']): string {
    switch (format) {
      case 'risum':
        return '🧩';
      case 'json':
  ...
  $: subtitle = card.artifactKind === 'module' ? (card.namespace ?? card.sourceFormat) : undefined;
</script>

<button
  type="button"
  class={`character-card character-card--${card.status}`}
  class:character-card--module={card.artifactKind === 'module'}
...
>
  ...
  <span class="card-body">
    ...
    {#if card.artifactKind === 'character' && card.tags.length > 0}
