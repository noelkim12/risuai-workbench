...
`.risutoggle`은 캐릭터 카드(charx), 모듈(module), 프리셋(preset)에서 사용하는 전용 표준 단일 파일 아티팩트 명세입니다. 이 아티팩트는 **CBS가 아닌 별도의 토글 전용 설정 언어(DSL)**를 사용한다는 점에 주의하십시오.
...
- **캐릭터/모듈 기본값**: 캐릭터 카드와 모듈 대상은 `toggle/<targetName>.risutoggle` 경로를 사용합니다.
